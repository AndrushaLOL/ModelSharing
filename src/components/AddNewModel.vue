            ...
            <v-stepper-items>
                ...
                <v-stepper-content step="3">
                    <v-card class="elevation-0">
                        ...
                        <v-btn flat color="indigo lighten-2">
                        <input  type="file" id='file_upload' accept="image/*" @change="filesChange($event.target.files)">
                        Select
                        </v-btn>
...
export default {
    ...
    methods: {
        ...
        addModel() {
            let that = this
            if (!this.file) return
            const uploadImage = storageImagesRef.child(this.file2.name).put(this.file2)
            const uploadTask = storageModelsRef.child(this.file.name).put(this.file)
            this.urlImg = uploadImage.snapshot.ref.getDownloadURL()
